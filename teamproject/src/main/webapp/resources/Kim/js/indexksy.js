...
const editorConfig = {
    ...
    simpleUpload: {
        uploadUrl: '/www/ckUploadFormAction',
        headers: {
...
$(function () {
    ...
    function showImage(fileCallPath) {
        ...
        $(".bigPicture").html(`<img src="/www/display?fileName=${fileCallPath}">`)
            .animate({
                width: "100%",
                height: "100%"
            }, 1000);
    ...
    function uploadFiles(files) {
        ...
        $.ajax({
			...
			$.each(rData, function (index, obj) {
			    ...
			    if (!obj.image) {
			        imgTag = `<a href='/www/download?fileName=${fileName}'>
			                        <img src='/www/resources/image/default.png' width='100'></a>`;
			    } else {
			        let path = `${obj.uploadPath}/s_${obj.uuid}_${obj.fileName}`; // 썸네일 경로 생성
        			let callPath = `${obj.uploadPath}/${obj.uuid}_${obj.fileName}`; // 원본 경로
			        imgTag = `<img class="imgImage" src="/www/display?fileName=${path}" 
			                        data-callpath="${callPath}">`;
			    }
			    let liTag = `<li
			                        data-filename="${obj.fileName}" // 수정된 부분: 파일 이름만 저장
			                        data-uploadpath="${obj.uploadPath}" // 경로만 저장
			                        data-uuid="${obj.uuid}"
			                        data-image="${obj.image}"
			                        >${imgTag} <br> ${obj.fileName} <span style="cursor:pointer;"
			                        data-filename="${obj.uploadPath}/${obj.uuid}_${obj.fileName}">&times;</span></li>`; // 수정된 부분: 전체 경로를 삭제용으로 저장
